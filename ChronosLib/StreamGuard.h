...
public:
  ...
  StreamFlagsGuard(Stream& s, FlagT flagBits) : StreamFlagsGuard(s) { s.flags(flagBits); }
...
template<typename Stream>
auto makeStreamFlagsGuard(Stream& s, typename StreamFlagsGuard<Stream>::FlagT flagBits) {
  return StreamFlagsGuard<Stream>(s, flagBits);
}

// Creates a scope that preserves the stream's fill character and restores it on exit.
template<typename OStream>
class StreamFillGuard {
public:
  using CharT = typename OStream::char_type;
  using TraitsT = typename OStream::traits_type;
  static_assert(std::is_base_of<std::basic_ostream<CharT, TraitsT>, OStream>::value);

  explicit StreamFillGuard(OStream& s) : m_stream(s), m_fillChar(s.fill()) {}
  StreamFillGuard(OStream& s, CharT fillChar) : m_stream(s), m_fillChar(s.fill(fillChar)) {}
...
template<typename OStream>
auto makeStreamFillGuard(OStream& s, typename StreamFillGuard<OStream>::CharT fillChar) {
  return StreamFillGuard<OStream>(s, fillChar);
}

// Creates a scope that preserves the stream's default width and restores it on exit.
template<typename Stream>
...
public:
  ...
  StreamWidthGuard(Stream& s, int cntWidth) : m_stream(s), m_cntWidth(s.width(cntWidth)) {}
